...
public class UserController : ControllerBase
{
 ...
 [HttpGet]
public async Task<ActionResult<TblUser>> GetUserByPassword(string password){
        string passwordRegex = @"^(?=.[a-zA-Z])(?=.\d)(?=.*[\W]).{8,13}$";
         if(!Regex.IsMatch(password,passwordRegex)){
            return BadRequest("Invalid Password format, the password should contain Uppercase,lowercase,digits and one specil character.ith should be 8 to 13 length");
        }
        var user=await _DBContext.TblUsers.FirstOrDefaultAsync(u =>u.Password==password);
        if(user==null){
            return NotFound();
        }
        return user;
